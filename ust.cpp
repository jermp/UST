...
string mapmode[] = {"basic",
                    "indegree_dfs",
                    "indegree_dfs_initial_sort_only",
                    "outdegree_dfs",
                    "outdegree_dfs_initial_sort_only",
                    "inverted_indegree_dfs",
                    "plus_indegree_dfs",
                    "random_dfs",
                    "node_assign",
                    "source_first",
...
struct node_sorter {
    int node;
    int sortkey;
    // bool operator() (struct node_sorter  i, struct node_sorter  j) { return
    // (i.sortkey<j.sortkey);}
};
bool sort_by_key(struct node_sorter i, struct node_sorter j) {
    return (i.sortkey < j.sortkey);
}
bool sort_by_key_inverted(struct node_sorter i, struct node_sorter j) {
    return (i.sortkey > j.sortkey);
}
...
vector<int>
    walkFirstNode;  // given a walk id, what's the first node of that walk
unordered_map<int, vector<edge_t> >
    sinkSrcEdges;  // int is the unitig id (old id)
...
void printBCALMGraph(vector<vector<edge_t> > adjList) {
    for (int i = 0; i < adjList.size(); i++) {
        ...
        for (edge_t edge : adjList.at(i)) {
            cout << boolToCharSign(edge.left) << ":" << edge.toNode << ":"
                 << boolToCharSign(edge.right) << ", ";
        }
        cout << endl;
    }
}
...
public:
    ...
    void indegreePopulate() {
        ...
        for (vector<edge_t> elist : adjList) {
            for (edge_t e : elist) {
                ...
                if (e.right == true) { global_plusindegree[e.toNode] += 1; }
                if (e.left == true) { global_plusoutdegree[xc] += 1; }
            }
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) { inOutCombo[make_pair(i, j)] = 0; }
        }
        for (int i = 0; i < V; i++) {
            pair<int, int> a;
            ...
            if (DBGFLAG == SINKSOURCE) { cout << i << "is "; }
            ...
            if (global_plusoutdegree[i] == 0 && global_plusindegree[i] != 0) {
                ...
                if (DBGFLAG == SINKSOURCE) { cout << "sink, "; }
            }
            if (global_plusindegree[i] == 0 && global_plusoutdegree[i] != 0) {
                ...
                if (DBGFLAG == SINKSOURCE) { cout << "source, "; }
            }
            ...
            if (global_indegree[i] == 0) {
                ...
                if (DBGFLAG == SINKSOURCE) { cout << "isolated, "; }
            }
            if (global_indegree[i] == 1) { onecount++; }

            if (DBGFLAG == SINKSOURCE) { cout << endl; }
        }
        ...
        for (vector<edge_t> elist : adjList) {
            ...
            if (FLG_NEWUB == true) {
                ...
                for (edge_t e_xy : elist) {  // central node: all neighbors of x
                    ...
                    // pair<int, bool> pairr;
                    for (edge_t e_from_y :
                         adjY) {  // check if this neighbor is speacial
                    ...
                    if (eligibleSp) { spNeighborCount[sLeft(e_xy)]++; }
                }
    ...
    void DFS_visit(int u) {
        if (ALGOMODE == BRACKETCOMP) {
            if (global_issinksource[u] == 1) {
                ...
                for (edge_t e : adju) { myvector.push_back(e); }
        ...
        while (!s.empty()) {
            ...
            if (color[x] == 'w') {
                ...
                if (ALGOMODE == INDEGREE_DFS) {
                    sort(adjx.begin(), adjx.end(),
                         [](const edge_t& lhs, const edge_t& rhs) {
                             return global_indegree[lhs.toNode] <
                ...
                if (ALGOMODE == OUTDEGREE_DFS) {
                    if (p_dfs[x] == -1) {
                        sort(adjx.begin(), adjx.end(),
                             [](const edge_t& lhs, const edge_t& rhs) {
                                 return global_outdegree[lhs.toNode] <
                        ...
                        sort(adjx.begin(), adjx.end(),
                             [](const edge_t& lhs, const edge_t& rhs) {
                                 return global_plusoutdegree[lhs.toNode] <
                                        global_plusoutdegree[rhs.toNode];
                             });
                    }
                }
                ...
                // For a white x
                // Consider 2 case:
                // Case 1. p[x] = -1, it can happen in two way, x is the first
                // one ever in this connected component, or no one wanted to
                // take x either way, if p[x] = -1, i can be representative of a
                // new node in new graph Case 2. p[x] != -1, so x won't be the
                // representative/head of a newHome. x just gets added to its
                // parent's newHome.
                ...
                if (p_dfs[x] == -1) {
                    ...
                    oldToNew[x].serial =
                        countNewNode++;  // countNewNode starts at 0, then keeps
                                         // increasing
                    ...
                    oldToNew[x].startPosWithKOverlap =
                        oldToNew[p_dfs[x]].endPosWithKOVerlap + 1;
                    oldToNew[x].pos_in_walk =
                        oldToNew[p_dfs[x]].pos_in_walk + 1;
                    if (u < K) {
                        oldToNew[x].endPosWithKOVerlap =
                            oldToNew[x].startPosWithKOverlap +
                            1;  // do we actually see this? yes
                        ...
                        oldToNew[x].endPosWithKOVerlap =
                            u - K +
                            (oldToNew[x]
                                 .startPosWithKOverlap);  // check correctness
                    }
                }
                ...
                for (edge_t yEdge : adjx) {  // edge_t yEdge = adjx.at(i);
                    ...
                    if (ALGOMODE == BRACKETCOMP) {
                        if (global_issinksource[y] == true) { continue; }
                    }

                    // Normal DFS
                    if (color[y] == 'w') { s.push(yEdge); }
                    ...
                    if (y == x) {
                        ...
                        // If x has space to take a child, meaning x is not
                        // saturated hunting for potential child

                        if (color[y] == 'w' && p_dfs[y] == -1) {
                            // y has white color & got no parent => means it's
                            // homeless, so let's see if we can take it as a
                            // child of x
                            // But just see if it is eligible to be a child,
                            // i.e. is it consistent (sign check)?
                            ...
                            if (p_dfs[x] == -1 && ALGOMODE != NODEASSIGN) {
                                ...
                                // case 2: child (=y) has grandparent, i.e. x's
                                // parent exists
                                nodeSign[y] = yEdge.right;
                                p_dfs[y] = x;
                                saturated[x] = true;  // found a child
                                ...
                                bool consistentEdge =
                                    (nodeSign[y] == yEdge.right &&
                                     (p_dfs[x] == -1 ||
                                      (p_dfs[x] != -1 &&
                                       nodeSign[x] == yEdge.left)));
                                if (p_dfs[y] == -1 && consistentEdge &&
                                    oldToNew[x].serial != oldToNew[y].serial) {
                                    // cout<<"x: "<<x<<":"
                                    // <<disSet.find_set(x)<<" "; cout<<"y:
                                    // "<<y<<":" <<disSet.find_set(y) <<endl;

                                    // not in same group already, prevent cycle
                                    if (disSet.find_set(x) !=
                                        disSet.find_set(y)) {
                                        nodeSign[x] = yEdge.left;
                                        nodeSign[y] = yEdge.right;
                                        p_dfs[y] = x;
    ...
    void DFS() {
        if (ALGOMODE == NODEASSIGN) {
            for (int i = 0; i < V; i++) {
                nodeSign[i] = true;
                if (global_plusindegree[i] <
                    global_indegree[i] - global_plusindegree[i]) {
                    nodeSign[i] = true;
                }
            }
        }
        ...
        //            }
        //            vector<struct node_sorter> myvector (sortStruct,
        //            sortStruct+V); sort (myvector.begin(), myvector.end(),
        //            sort_by_key_inverted); copy(myvector.begin(),
        //            myvector.end(), sortStruct);
        //        }
        ...
        if (ALGOMODE == INDEGREE_DFS || ALGOMODE == INDEGREE_DFS_1) {
            ...
            if (DBGFLAG == INDEGREEPRINT) {
                ...
                for (int i = 0; i < V; i++) {
                    cout << sortStruct[i].node << "->" << sortStruct[i].sortkey
                         << endl;
                }
            }
        }
        ...
        for (int j = 0; j < V; j++) {
            int i;
            if (ALGOMODE == OUTDEGREE_DFS || ALGOMODE == OUTDEGREE_DFS_1 ||
                ALGOMODE == INDEGREE_DFS || ALGOMODE == INDEGREE_DFS_1 ||
                ALGOMODE == SOURCEFIRST) {
                i = sortStruct[j].node;
            } else {
                i = j;
            }
        ...
        for (int i = 0; i < countNewNode; i++) { merged[i] = false; }
                ...
                if (gmerge.fwdVisited[p.first] == false) {
                    ...
                    if (gmerge.fwdVisited.count(tonode) > 0) {
                        while (gmerge.fwdVisited[tonode] == false) {
                            ...
                            lst.push_back(tonode);
                            if (gmerge.fwdVisited.count(tonode) == 0) break;
                        }
                    }
                    if (gmerge.bwdVisited.count(fromnode) > 0) {
                        while (gmerge.bwdVisited[fromnode] == false) {
                            ...
                            if (gmerge.bwdVisited.count(fromnode) == 0) break;
                        }
                    }

                    int headOfThisWalk =
                        walkFirstNode[lst.at(0)];  // CHECK AGAIN
                    assert(!lst.empty());
        ...
        for (int uid = 0; uid < V; uid++) {
            ...
            sorter.push_back(
                make_tuple(uid, nd.finalWalkId, nd.pos_in_walk, nd.isTip));
        }
        ...
        string uidSeqFilename =
            "uidSeq.usttemp";  //"uidSeq"+ mapmode[ALGOMODE] +".txt"
        ...
        for (fourtuple n : sorter) {
            ...
            //                        int isTip = get<3>(n);
            //                        cout<<uid<<" " <<finalWalkId<<"
            //                        "<<pos_in_walk<<" "<<isTip<<"
            //                        "<<oldToNew[uid].isWalkEnd<< " was merged:
            //                        "<< merged[oldToNew[uid].finalWalkId]<<
            //                        endl;
        ...
        if (FLG_ABUNDANCE) {
            system(("awk '(NR%2)' " + UNITIG_FILE +
                    " | cut -f 5 -d ':' | cut -f 1 -d 'L' > count.usttemp")
                       .c_str());  // get a separate count file
            system(
                "paste -d' ' uidSeq.usttemp seq.usttemp count.usttemp > "
                "merged.usttemp ");
        ...
        ifstream sequenceStringFile;
        if (FLG_ABUNDANCE) {  // option -a 1
            sequenceStringFile.open("merged.usttemp");
        } else {
            system("cat  merged.usttemp  | cut -d' ' -f3 >  seq.usttemp");
            sequenceStringFile.open("seq.usttemp");
        }
        ...
        if (100 == 100) {
            ...
            for (fourtuple n : sorter) {
                ...
                if (FLG_ABUNDANCE) {
                    uint64_t count = 0;
                    uint64_t unitigString_begin = 0;
                    for (uint64_t i = 0; i != sequenceFromFile.size(); ++i) {
                        if (sequenceFromFile[i] == ' ') {
                            count += 1;
                            if (count == 2) {
                                unitigString_begin = i + 1;
                            } else if (count == 3) {
                                unitigString = sequenceFromFile.substr(
                                    unitigString_begin, i - unitigString_begin);
                                countString = sequenceFromFile.substr(
                                    i + 1, sequenceFromFile.size() - i - 1);
                                break;
                            }
                        }
                    }

                    // remove possible white spaces at the end of countString
                    while (countString.back() == ' ') {
                ...
                if (FLG_ABUNDANCE) {
                    if (nodeSign[uid] == false) {
                        unitigString = reverseComplement(unitigString);
                    }
                } else {
                    if (nodeSign[uid] == false) {
                ...
                if (finalWalkId != lastWalk) {
                    if (lastWalk != -1) {
                        // print previous walk
                        // ustOutputFile<<">"<<lastWalk << " " << uid<<" "
                        // <<finalWalkId<<" "<<pos_in_walk<<" "<<endl;
                        if (walkString.length() >= K) {
                            if (FLG_ABUNDANCE) {
                                // example header: '>12 LN:i:41 ab:Z:2 2 2 2 2 2
                                // 2 2 2 2 2'
                                ustOutputFile << ">" << walk_id
                                              << " LN:i:" << walkString.length()
                                              << " ab:Z:" << walkStringCounts
                                              << endl;
                            } else {
                                ustOutputFile << ">" << endl;
                            }
                ...
                if (FLG_ABUNDANCE) walkStringCounts += countString + ' ';

                // ustOutputFile<<">"<<uid<<" " <<finalWalkId<<"
                // "<<pos_in_walk<<endl;
            }
...
void formattedOutputForwardExt(Graph& G) {
    ...
    for (int newNodeNum = 0; newNodeNum < G.countNewNode; newNodeNum++) {
        myfile << '>' << newNodeNum
               << " LN:i:" << newSequences[newNodeNum].length() << " ";
        myfile << endl;
    ...
    // myfile << '>' << newNodeNum <<">0 LN:i:13 KC:i:12 km:f:1.3  L:-:0:-
    // L:-:2:-  L:+:0:+ L:+:1:- " ;
    myfile.close();
    plainfile.close();
}

int read_unitig_file(const string& unitigFileName,
                     vector<unitig_struct_t>& unitigs) {
    ifstream unitigFile;
    ...
    do {
        ...
        if (FLG_ABUNDANCE) {
            //>3 LN:i:24 ab:Z:10 10 10 10 10 7 7 7 7 7 3 3 3 3   L:-:0:+ L:-:1:+
            // L:+:0:-
            edgesline[0] = '\0';
            ...
            if (unitig_struct.ln < smallestK) { smallestK = unitig_struct.ln; }
            ...
            sscanf(line.c_str(), "%*c %d %s  %s  %s %[^\n]s",
                   &unitig_struct.serial, lnline, kcline, kmline, edgesline);
        ...
        while (getline(ss, line, ' ')) {
            if (delSpaces(line).length() != 0) {
                if (DBGFLAG == VERIFYINPUT) { cout << line << endl; }

                sscanf(line.c_str(), "%*2c %c %*c %d  %*c  %c", &c1, &nodeNum,
                       &c2);  // L:-:0:-
                edge_t newEdge;
    ...
    if (smallestK > K) {
        cout << "\n :::: :::: :::: :::: !!!!!!!!! WARNING !!!!!!!!!!! :::: "
                ":::: :::: ::::"
             << endl;
        cout << "The length of the smallest string we found was " << smallestK
             << ". Please make sure you are using the correct value of 'k' to "
...
int main(int argc, char** argv) {
    ...
    if (DEBUGMODE == false) {
        while ((c = getopt(argc, argv, "i:k:a:")) != -1) {
            switch (c) {
                case 'a':
                    if (optarg) {
                        if (strcmp(optarg, "0") == 0 ||
                            strcmp(optarg, "1") == 0) {
                            FLG_ABUNDANCE =
                                static_cast<bool>(std::atoi(optarg));
                        } else {
                            fprintf(stderr,
                                    "Error: use either -a 0 or -a 1 \n");
                            exit(EXIT_FAILURE);
                        }
                    }
                ...
                case 'i':
                    ...
                    //                    if(optarg) {
                    //                        ALGOMODE =
                    //                        static_cast<ALGOMODE_T>(std::atoi(optarg));
                    //                    }
                    //                    break;
                    // case 'd':
                    // if(optarg) {
                ...
                case 'k':
                    if (optarg) {
                        ...
                        if (K <= 0) {
                            fprintf(stderr,
                                    "Error: Specify a positive k value.\n");
                            exit(EXIT_FAILURE);
                        }
                    } else {
                        fprintf(stderr,
                                "Usage: %s -k <kmer size> -i <input-file-name> "
                                "[-a <0: for no counts, 1: to output separate "
                                "count, default = 0>]\n",
                                argv[0]);
                        exit(EXIT_FAILURE);
                    }
                ...
                default:  //
                    fprintf(stderr,
                            "Usage: %s -k <kmer size> -i <input-file-name> [-a "
                            "<0: for no counts, 1: to output separate count, "
                            "default = 0>]\n",
                            argv[0]);
                    exit(EXIT_FAILURE);
            }
        ...
        if (K == 0 || strcmp(nvalue, "") == 0) {
            fprintf(stderr,
                    "Usage: %s -k <kmer size> -i <input-file-name> [-a <0: for "
                    "no counts, 1: to output separate count, default = 0>]\n",
                    argv[0]);
            exit(EXIT_FAILURE);
        }
    ...
    globalStatFile.open((getFileName(UNITIG_FILE) + ".stats.txt").c_str(),
                        std::fstream::out | std::fstream::app);

    double startTime = readTimer();
    cout << "## Please wait while we read input file.... " << UNITIG_FILE
         << ": k = " << K << endl;
    ...
    int E_bcalm = 0;
    for (int i = 0; i < G.V; i++) { E_bcalm += adjList[i].size(); }
    int V_bcalm = G.V;
    int numKmers = 0;
    int C_bcalm = 0;
    ...
    //        cout<<"Total Nodes: "<<V<<" Edges: "<<E<<" K-mers:
    //        "<<numKmers<<endl;
    //    }

    cout << "## Please wait while we gather info about lower bound.... "
         << endl;
    double time_a = readTimer();
    G.indegreePopulate();

    cout << "Done. TIME to gather information about unitig graph: "
         << readTimer() - time_a << " sec." << endl;

    int walkstarting_node_count =
        ceil((sharedparent_count + sink_count + source_count) / 2.0) +
        isolated_node_count;
    int charLowerbound =
        C_bcalm - (K - 1) * (G.V - walkstarting_node_count * 1.0);
    float upperbound =
        (1 - ((C_bcalm - (K - 1) * (G.V - walkstarting_node_count * 1.0)) /
              C_bcalm)) *
        100.0;
...
//           %.2f%%\t",
    ...
    globalStatFile << "K"
                   << "=" << K << endl;
    globalStatFile << "N_KMER"
                   << "=" << numKmers << endl;
    globalStatFile << "V_BCALM"
                   << "=" << V_bcalm << endl;
    globalStatFile << "E_BCALM"
                   << "=" << E_bcalm << endl;
    globalStatFile << "C_BCALM"
                   << "=" << C_bcalm << endl;
    globalStatFile << "C_LB"
                   << "=" << charLowerbound << endl;
    globalStatFile << "V_LB"
                   << "=" << walkstarting_node_count << endl;
    globalStatFile << "N_ISOLATED"
                   << "=" << isolated_node_count << endl;
    globalStatFile << "N_SINK"
                   << "=" << sink_count << endl;
    globalStatFile << "N_SOURCE"
                   << "=" << source_count << endl;

    if (FLG_NEWUB)
        globalStatFile << "N_SPECIAL_NEW"
                       << "=" << sharedparent_count << endl;
    if (!FLG_NEWUB)
        globalStatFile << "N_SPECIAL_OLD"
                       << "=" << sharedparent_count << endl;

    // OPTIONAL;DERIVABLE
    globalStatFile << "BITSKMER_LB"
                   << "=" << (charLowerbound * 2.0) / numKmers << endl;
    globalStatFile << "PERCENT_UB"
                   << "=" << upperbound << "%" << endl;
    globalStatFile << "PERCENT_N_SPECIAL"
                   << "=" << sharedparent_count * 100.0 / V_bcalm << "%"
                   << endl;

    for (auto i = inOutCombo.begin(); i != inOutCombo.end(); i++) {
        // printf("%.2f%%\t", (i->second)*100.0/V_bcalm);
        globalStatFile << "PERCENT_DEGREE_" << (i->first).first << "_"
                       << (i->first).second << "="
                       << (i->second) * 100.0 / V_bcalm << "%" << endl;
    }
    //printf("%.2f%%\t\
           %.2f%%\t",
    ...
    // OPTIONAL; derivable
    globalStatFile << "PERCENT_N_ISOLATED"
                   << "=" << isolated_node_count * 100.0 / V_bcalm << "%"
                   << endl;
    globalStatFile << "PERCENT_N_DEADEND"
                   << "=" << (sink_count + source_count) * 100.0 / V_bcalm
                   << "%" << endl;
...
//           %.2f\t",
    ...
    // globalStatFile << "USTITCH_MODE" <<  "=" << mapmode[ALGOMODE].c_str() <<
    // endl;
    globalStatFile << "V_USTITCH_" << mapmode[ALGOMODE].c_str() << "="
                   << V_ustitch << endl;
    globalStatFile << "PERCENT_C_SAVED_" << mapmode[ALGOMODE].c_str() << "="
                   << percent_saved_c << "%" << endl;
    globalStatFile << "PERCENT_C_GAP_WITH_UB_" << mapmode[ALGOMODE].c_str()
                   << "=" << upperbound - percent_saved_c << "%" << endl;
    globalStatFile << "C_USTITCH_" << mapmode[ALGOMODE].c_str() << "="
                   << C_ustitch << endl;
    globalStatFile << "BITSKMER_USTITCH_" << mapmode[ALGOMODE].c_str() << "="
                   << ustitchBitsPerKmer << endl;
    globalStatFile << "TIME_READINPUT_SEC_" << mapmode[ALGOMODE].c_str() << "="
                   << TIME_READ_SEC << endl;
    globalStatFile << "TIME_TOTAL_SEC_" << mapmode[ALGOMODE].c_str() << "="
                   << TIME_TOTAL_SEC << endl;

    globalStatFile.close();

    cout << "------------ UST completed successfully. Output is in file "
         << OUTPUT_FILENAME << " ------------" << endl;
    cout << "Total number of unique " << K << "-mers "
         << "= " << numKmers << endl;
    cout << "Lower bound on any SPSS representation "
         << "= " << (charLowerbound)*1.0 / numKmers << " neucleotide/k-mer"
         << endl;
    cout << "Size of UST output"
         << "= " << ustitchBitsPerKmer / 2.0 << " neucleotide/k-mer" << endl;
    // cout << "Gap with lower bound" << "= " << upperbound - percent_saved_c <<
    // "%" << endl;
